...
BOOL ZFrustum::Make(D3DXMATRIXA16 * pmatViewProj)
{
	...
	// 투영행렬까지 거치면 모든 3차원 월드 좌표의 점은 (-1, -1, 0) ~ (1, 1, 1)사이의 값으로 바뀝니다.
	// m_vtx에 이 동차공간의 경계값을 넣어둡니다.
	 ...
	 * Vertex_최종 = Vertex_world * Matrix_view * Matrix_Proj 입니다.
	 * Vertex_최종 = Vertex_world * (Matrix_view * Matrix_Proj)에서
	 * 역행렬 (Matrix_view * Matrix_Proj)^-1를 양변에 곱하면
	 * Vertex_최종 * 역행렬(Matrix_view * Matrix_Proj)^-1 = Vertex_wolrd가 됩니다.
	 * 그러므로, m_vtx * matInv = Vertex_world가 되어, 월드 좌표계의 프러스텀 좌표를
	 * 얻을 수 있습니다.
	 *=============================================================================*/
	for (i = 0; i < 8; i++)
	{
	...
	// 0번과 5번은 프러스텀 중 near평면의 좌측상단과 우측하단이므로, 둘의 좌표를 더해서 2로 나누면
	// 카메라의 좌표를 얻을 수 있습니다.(정확히 일치하는 것은 아닙니다.)
	m_vPos = (m_vtx[0] + m_vtx[5]) / 2.0f;

	// 얻어진 월드 좌표로 프러스텀 평면을 만듭니다.
	// 벡터가 프러스텀 안쪽에서 바깥쪽으로 나가는 평면들입니다.
...
BOOL ZFrustum::IsIn(D3DXVECTOR3 * pv)
{
	...
	// 현재는 left, right, far plane만 적용합니다.
	for (i = 0; i < 6; i++)
	{
