...
HRESULT Vertex::InitVB(LPDIRECT3DDEVICE9 & device)
{
	...
	for (DWORD i = 0; i < 50; i++)
	{
		...
		pVertices[2 * i + 0].normal = D3DXVECTOR3(sinf(theta), 0.0f, cosf(theta));		// 실린더의 아래쪽 원통 법선 벡터
		pVertices[2 * i + 1].position = D3DXVECTOR3(sinf(theta), 1.0f, cosf(theta));	// 실린더의 위쪽 원통 좌표
		pVertices[2 * i + 1].normal = D3DXVECTOR3(sinf(theta), 0.0f, cosf(theta));		// 실리넏의 위쪽 원통 법선 벡터
	}
