...
DWORD						g_cxHeight		= 0;		// 텍스처의 가로 크기
DWORD						g_czHeight		= 0;		// 텍스처의 세로 크기
...
HRESULT InitVB()
{
	...
	g_pvHeightMap = new D3DXVECTOR3[g_cxHeight * g_czHeight];	//높이맵 배열 생성
...
HRESULT ProcessFrustumCull()
{
	...
	BOOL		b[4];		// 임시로 저장할 frustum culling 결과 값
	MYINDEX		idx;
	MYINDEX*	pI;
	...
	for (DWORD z = 0; z < g_czHeight - 1; z++)
	{
		...
		{
			i[0] = (z * g_czHeight + x);				// 좌측 상단
			i[1] = (z * g_czHeight + x + 1);			// 우측 상단
			i[2] = ((z + 1) * g_czHeight + x);			// 좌측 하단
			i[3] = ((z + 1) * g_czHeight + x + 1);		// 우측 하단

			b[0] = g_pFrustum->IsIn(&g_pvHeightMap[i[0]]);		// 좌측 상단 정점이 Frustum안에 있는가?
			b[1] = g_pFrustum->IsIn(&g_pvHeightMap[i[1]]);		// 우측 상단 정점이 Frustum안에 있는가?
			b[2] = g_pFrustum->IsIn(&g_pvHeightMap[i[2]]);		// 좌측 하단 정점이 Frustum안에 있는가?
			// 셋중에 하나라도 frustum안에 있으면 렌더링합니다.
			if (b[0] | b[1] | b[2])
			{
				idx._0 = i[0];
				idx._1 = i[1];
				idx._2 = i[2];
				*pI++ = idx;
				g_nTriangles++;		// 랜더링할 삼각형 개수 증가
			}

			b[2] = g_pFrustum->IsIn(&g_pvHeightMap[i[2]]);		// 좌측 하단 정점이 Frustum안에 있는가?
			b[1] = g_pFrustum->IsIn(&g_pvHeightMap[i[1]]);		// 우측 상단 정점이 Frustum안에 있는가?
			b[2] = g_pFrustum->IsIn(&g_pvHeightMap[i[3]]);		// 우측 하단 정점이 Frustum안에 있는가?
			// 셋중에 하나라도 frustum안에 있으면 렌더링합니다.
			if (b[2] | b[1] | b[3])
			{
				idx._0 = i[2];
...
VOID Animate()
{
	...
	ProcessFrustumCull();					// Frustum정보를 기반으로, 보여질 삼각형의 인덱스를 만듭니다.
