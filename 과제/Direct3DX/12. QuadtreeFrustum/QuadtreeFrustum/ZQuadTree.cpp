...
void ZQuadTree::FrustumCull(TERRAINVERTEX * pHeightMap, ZFrustum * pFrustum)
{
	...
	switch (ret)
	{
	case ZQuadTree::FRUSTUM_OUT:					// Frustum에 완전 포함, 하위 노드 검색 필요없음
		m_bCulled = TRUE;
		break;
	case ZQuadTree::FRUSTUM_PARTIALLY_IN:		// Frustum에 일부 포함, 하위 노드 검색 필요함
		m_bCulled = FALSE;
		break;
	case ZQuadTree::FRUSTUM_COMPLETELY_IN:	// Frustum에 완전 벗어남, 하위 노드 검색 필요없음
		m_bCulled = FALSE;
		break;
	}
