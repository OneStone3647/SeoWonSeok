...
void CardManager::Init(HWND hWnd, HINSTANCE hInst)
{
	...
	{
		m_Card[i].Init(hdc, m_hInst, IDB_BITMAP1 + i);		// 각 카드들 초기화
	}
...
// 특정 카드를 그린다.
void CardManager::DrawCard(HDC hdc, int cardNum, int x, int y)
{
	m_Card[cardNum].Draw(hdc, Card_X + (m_Card[cardNum].GetCardSize().cx * x), Card_Y + (m_Card[cardNum].GetCardSize().cy * y));
}
...
void CardManager::CheckCard(HWND hWnd, int mouse_X, int mouse_Y)
{
	...
	if ((mouse_X >= Card_X && mouse_X <= Card_X + (Card_Width * 5)) && (mouse_Y >= Card_Y && mouse_Y <= Card_Y + (Card_Height * 4)))
	{
		...
		if (m_FirstCardListIndex != NULL && m_SecondCardListIndex != NULL)
		{
			...
			{
				m_bStartGame = false;
				// 같은 디자인이 아닐경우 1초 뒤 카드의 뒷면을 그린다.
				SetTimer(hWnd, 2, 1000, StopClick);
			}
		}
		CheckContinue(hWnd);
	}
...
void CardManager::StopClick(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	GetInstance()->SetStartGame(true);
	GetInstance()->ReturnCard(hWnd);
	KillTimer(hWnd, 2);
}

void CardManager::ReturnCard(HWND hWnd)
{
	m_CardList[m_FirstCardListIndex - 1].bIsOpen = false;
	m_CardList[m_SecondCardListIndex - 1].bIsOpen = false;
	DrawCard(hdc, 10, m_CardList[m_FirstCardListIndex - 1].X, m_CardList[m_FirstCardListIndex - 1].Y);
	DrawCard(hdc, 10, m_CardList[m_SecondCardListIndex - 1].X, m_CardList[m_SecondCardListIndex - 1].Y);
	m_FirstCardListIndex = NULL;
	m_SecondCardListIndex = NULL;
	CheckContinue(hWnd);
}

void CardManager::CheckContinue(HWND hWnd)
{
