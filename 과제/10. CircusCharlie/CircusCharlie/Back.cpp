...
void Back::Draw(float CameraX)
{
	float test = 100.0f;
	// 바닥 그리기
	for (int i = 0; i < 7; i++)
	{
		m_Back.Draw(m_BackDC, i * m_Back.GetSize().cx * 2.2f - CameraX + test, 270, 2.3f, 2.3f);
	}
	...
	for (int i = 0; i < 11; i++)
	{
		...
		m_Normal[m_NormalIndex].Draw(m_BackDC, i * m_Normal[m_NormalIndex].GetSize().cx * 1.45f - CameraX + test, 176);
	}

	// 코끼리 그리기
	m_Deco.Draw(m_BackDC, m_Deco.GetSize().cx * 1.45f - CameraX + test, 172);

	// 미터기 그리기
	TCHAR miter[256];
	m_MiterCount = 100;
	m_Miter.Draw(m_BackDC, m_Miter.GetSize().cx * 5.0f - CameraX + 20.0f + test, 600);
	wsprintf(miter, TEXT("   %d   "), m_MiterCount);
	TextOut(m_BackDC, m_Miter.GetSize().cx * 5.4f - CameraX + 20.0f + test, 615, miter, strlen(miter));
}
