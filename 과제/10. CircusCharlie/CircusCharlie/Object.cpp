...
void Object::SetCollision(Bitmap * bitmap, float Size, float CameraX)
{
	m_Collision.left = m_X - CameraX;
	m_Collision.right = m_X + bitmap->GetSize().cx * Size - CameraX;
	m_Collision.top = m_Y;
	m_Collision.bottom = m_Y + bitmap->GetSize().cy * Size;
}
